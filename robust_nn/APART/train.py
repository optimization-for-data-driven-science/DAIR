...
def test(args, model, device, epoch, test_loader):
    global writer
    adversary_test = list()
    adversary_test = [GradientSignAttack(model,loss_fn =  nn.CrossEntropyLoss(),eps=8/255),
    LinfPGDAttack( \
    ...
    loss_fn=nn.CrossEntropyLoss(), nb_iter = 10, eps=8/255, eps_iter= 0.007),
    ]
    model.eval()
    mapper = ["fgsm", "pgd20", "pgd40", "ifgsm"]
    for idx, attack in enumerate(adversary_test):
        test_loss = 0
        correct = 0
        total = 0
