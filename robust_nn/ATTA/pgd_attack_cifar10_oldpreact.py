...
def eval_adv_test_whitebox(model, device, test_loader, adversary_test):
    """
    ...
    for idx, adversary in enumerate(adversary_test):
        ...
        for data, target in test_loader:
            ...
            advdata = adversary.perturb(data, target)
            output = model(advdata)
            test_loss += F.cross_entropy(output, target, size_average=False).item()
            pred = output.max(1, keepdim=True)[1]
            correct += pred.eq(target.view_as(pred)).sum().item()
        test_loss /= len(test_loader.dataset)
        print(mapper[idx] + ' Test: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)'.format(
...
def main():
    if args.white_box_attack:
        ...
        adversary_test = [
        GradientSignAttack(model,loss_fn =  nn.CrossEntropyLoss(),eps=8/255),
        LinfPGDAttack( \
        model, loss_fn=nn.CrossEntropyLoss(), eps=8/255, \
        nb_iter=20, eps_iter=2/255, rand_init=True, clip_min=0.0, \
        clip_max=1.0, targeted=False)]

        eval_adv_test_whitebox(model, device, test_loader, adversary_test=adversary_test)
