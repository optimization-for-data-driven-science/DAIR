...
class EntityMutator(object):
    ...
    def create_gibberish_entity(self, entity_name):
        ...
        for _ in range(num_entities):
            ...
            gibberish_entities.append("".join(self.rng.choice(self.ALLOWLIST_GIBBERISH, str_len)))
...
class MultiWOZv22DSTTeacher(DialogTeacher):
    ...
    def __init__(self, opt: Opt, shared=None, *args, **kwargs):
        ...
        self.mutator = {"scramble": mutator.scramble,
        ...
        for i in range(1, 128):
            with open(ff + "train/dialogues_%03d.json" % i, "r") as f:
                for line in f:
                    line = line.strip()
                    if "\"restaurant_name\":" in line and not "\"restaurant_name\": [" in line:
                        word = line.split("\": \"")[1][:-2]
                ...
                if 'slot_values' in frame['state']:
                    for domain_slot_type in frame['state']['slot_values']:
                        for slot_value in frame['state']['slot_values'][domain_slot_type]:
                            ...
                            belief_state = f"{domain} {slot_type} {slot_value.lower()}" + "//" + frame["service"] + "//" + domain_slot_type + "//" + slot_value.lower()
    ...
    def _extract_slot_from_string(self, slots_string):
        ...
        return slots_list, per_domain_slot_lists, named_entity_slot_lists, named_entity_slot_interested_lists
    ...
    def custom_evaluation(
        ...
        # print("/" * 10)
        # print(slots_truth_named_entity_interested)
        # print(slots_pred_named_entity_interested)
        # print("/" * 10)



        # print(slots_truth, "?1")
        # print(slots_truth_per_domain)
        # print(slots_truth_named_entity, "?2")

        # print()

        # print(slots_pred, "?3")
        # print(slots_pred_per_domain)
        # print(slots_pred_named_entity, "?4")
        ...
        if self.opt["new_metric"] == False:
            if (len(slots_truth_named_entity_interested) > 0) or (len(slots_pred_named_entity_interested) > 0):

                self.metrics.add("jga_c", AverageMetric(set(slots_truth) == set(slots_pred)))
                ...
                for predicted_slot in slots_pred:
                    self.metrics.add("slot_p_c", AverageMetric(predicted_slot in slots_truth))
        ...
        if self.opt["new_metric"] == True:
            ...
            self.flag_compute2 = self.flag_compute2 or (len(slots_truth_named_entity_interested) > 0)
            self.flag_compute2 = self.flag_compute2 or (len(slots_pred_named_entity_interested) > 0)

            slot_p_curr = True 
            slot_r_curr = True

            for gt_slot in slots_truth_named_entity_interested:
                slot_r_curr = slot_r_curr and (gt_slot in slots_pred_named_entity_interested)

            for pred_slot in slots_pred_named_entity_interested:
                slot_p_curr = slot_p_curr and (pred_slot in slots_truth_named_entity_interested)

            jga_curr = (set(slots_pred_named_entity_interested) == set(slots_truth_named_entity_interested))

            if self.flag_compute == 1:


                self.metrics.add("NEW_METRIC_SLOT_P", AverageMetric(slot_p_curr == self.slot_p_prev))
                self.metrics.add("NEW_METRIC_SLOT_R", AverageMetric(slot_r_curr == self.slot_r_prev))
                self.metrics.add("NEW_METRIC_JGA", AverageMetric(jga_curr == self.jga_prev))
                ...
                if self.flag_compute2 == True:
                    ...
                    self.metrics.add("NEW_METRIC_SLOT_P_C", AverageMetric(slot_p_curr == self.slot_p_prev))
                    self.metrics.add("NEW_METRIC_SLOT_R_C", AverageMetric(slot_r_curr == self.slot_r_prev))
                    self.metrics.add("NEW_METRIC_JGA_C", AverageMetric(jga_curr == self.jga_prev))
                    if self.jga_prev == True:
                        self.metrics.add("DAIM_C", AverageMetric(jga_curr == self.jga_prev))
                    self.flag_compute2 = False
        ...
        for predicted_slot in slots_pred_named_entity:
            ...
            self.metrics.add(
                f"{curr_domain}_ne/hallucination", AverageMetric(not (ne.strip() in teacher_action.get("text")))
            )
                    ...
                    for kk in range(10):
                        ...
                        for bs in tmp_cum_belief_states:
                            ...
                            if bs_[2] in self.entity_bank[self.opt["entity"]]:
                                ...
                                if bs_[3] in tmp_text:
                                    ...
                                    new_slot_value = random.choice(self.entity_bank[self.opt["entity"]][bs_[2]][fold])
                                    tmp_text = tmp_text.replace(bs_[3], new_slot_value)
                                    processed_cum_belief_states.append(bs_[0].replace(bs_[3], new_slot_value))
                        ...
                        for (x, x_) in replaced_names:
                            for idx in range(len(processed_cum_belief_states)):
                                processed_cum_belief_states[idx] = processed_cum_belief_states[idx].replace(x, x_)
                ...
                for turn in dialog['turns']:
                    ...
                    if curr_speaker == "<user>":
                        ...
                        if self.opt["new_metric"] == True:
                                ...
                                for bs in tmp_cum_belief_states:
                                    ...
                                    if bs_[2] in self.entity_bank[self.opt["entity1"]]:
                                        ...
                                        if bs_[3] in tmp_text:
                                            ...
                                            new_slot_value = random.choice(self.entity_bank[self.opt["entity1"]][bs_[2]][fold])
                                            tmp_text = tmp_text.replace(bs_[3], new_slot_value)
                                            processed_cum_belief_states.append(bs_[0].replace(bs_[3], new_slot_value))
                                            replaced_names.append((bs_[3], new_slot_value))
                            ...
                            for bs in tmp_cum_belief_states:
                                ...
                                if bs_[2] in self.entity_bank[self.opt["entity2"]]:
                                    ...
                                    if bs_[3] in tmp_text:
                                        ...
                                        new_slot_value = random.choice(self.entity_bank[self.opt["entity2"]][bs_[2]][fold])
                            ...
                            if self.opt["entity"] == "multiwoz" and self.opt["uniform"] == False:
                                cum_belief_states_ = [item.split("//")[0] for item in cum_belief_states]
                                cum_belief_states_ = list(set(cum_belief_states_))
                                cum_belief_states_.sort()
                                iidx += 1
