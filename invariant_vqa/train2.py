...
def run(args,net, loader, optimizer, tracker, train=False, prefix='', epoch=0, dataset=None):
    ...
    for batch in tq:                                            #for v, q, a, idx, img_id, ques_id, q_len in tq:
        ...
        if train:            
            ...
            acc2 = utils.batch_accuracy(out2.data, a_e.data).cpu()
    ...
    print(prefix,' pos2neg :', fmt(pos2neg/total_edits), 'neg2pos :', fmt(neg2pos/total_edits), 'neg2neg :', fmt(neg2neg/total_edits), 'preds. flipped:', fmt((pos2neg + neg2pos + neg2neg)/total_edits))
...
def main(args):
    ...
    if config.model_type == 'no_attn':
        ...
        log = torch.load('./models/lambda_1_gamma_half/epoch_38.pth')
        tokens = train_loader.dataset.num_tokens
